...
const [hasLiked, setHasLiked] = useState(false);
  ...
  const [lastClickedTooltip, setLastClickedTooltip] = useState<'email' | 'address' | null>(null);
    ...
    // Use social.email if available, otherwise fall back to author.email
    const displayEmail = social.email || author.email;
    const socialLinks = [
    ...
    return (
        ...
        >
            {/* Profile Image */}
            <div className="w-54 h-54 mx-auto mb-6 rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105">
                <Image
                    src={author.avatar}
                    alt={author.name}
                    width={256}
